...
def set_stat_ranked() -> Tuple[Stat, Stat]:
    """Create two stat objects to test with mod ranks."""
    stats = [
        Stats(
            datetime="2021-05-16T21:00:00.000+00:00",
            volume=3,
...
def set_stat_ranked_difflen() -> Tuple[Stat, Stat]:
    """Create two stat objects to test with mod ranks with different lengths."""
    stats = [
        Stats(
            datetime="2021-05-16T21:00:00.000+00:00",
            volume=3,
...
def test_merge_ranked() -> None:
    """Test merging to `Stat` object with ranked stats."""
    stat, stat2 = set_stat_ranked()
    ...
    assert stat.dates == correct_dates
    assert stat.volumes == [3, 1, 101, 103, 11, 11]
    assert stat.live_stat_buy.volumes == [102]
    assert stat.live_stat_sell.volumes == [100]
    stat_test, stat2 = set_stat_ranked()
    stat_test.merge(stat2, newer=False)
    assert stat_test.dates == correct_dates
    assert stat_test.volumes == [3, 1, 2, 2, 11, 11]
    assert stat_test.live_stat_buy.volumes == [1]
    assert stat_test.live_stat_sell.volumes == [2]
    stat, stat_test2 = set_stat_ranked()
    stat_test2.merge(stat)
    assert stat_test2.dates == correct_dates
    assert compare(stat_test2, stat_test)


def test_merge_ranked_difflen() -> None:
    """Test merging to `Stat` object with ranked stats with different lengths."""
    stat, stat2 = set_stat_ranked_difflen()
    stat.merge(stat2)
    correct_dates = [
        datetime.datetime(year=2021, month=5, day=16, hour=21, tzinfo=timezone.utc),
        datetime.datetime(year=2021, month=5, day=16, hour=21, tzinfo=timezone.utc),
    ...
    assert stat.dates == correct_dates
    assert stat.volumes == [3, 1, 101, 103, 11]
    assert stat.live_stat_buy.volumes == [102]
    assert stat.live_stat_sell.volumes == [100]
    stat_test, stat2 = set_stat_ranked_difflen()
    stat_test.merge(stat2, newer=False)
    assert stat_test.dates == correct_dates
    assert stat_test.volumes == [3, 1, 2, 2, 11]
    assert stat_test.live_stat_buy.volumes == [1]
    assert stat_test.live_stat_sell.volumes == [2]
    stat, stat_test2 = set_stat_ranked_difflen()
    stat_test2.merge(stat)
    assert stat_test2.dates == correct_dates
    assert compare(stat_test2, stat_test)
